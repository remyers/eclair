...
import fr.acinq.eclair.channel.fsm.{Channel, QuiesceChannel}
...
class NormalQuiescentStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  private def receiveSettlementCommand(f: FixtureParam, c: SettlementCommandEnum, sendInitialStfu: Boolean, resetConnection: Boolean = false): Unit = {
    ...
    if (resetConnection) {
      ...
      // alice sends a warning if an error occurs while waiting for stfu from bob
      alice ! QuiesceChannel.LocalFailure(new ChannelException(channelId(bob), "timed out waiting for remote stfu"))
  ...
  test("recv stfu when there are pending local changes") { f =>
    ...
    crossSign(bob, alice, bob2alice, alice2bob)
    eventually(assert(bob.stateData.asInstanceOf[ChannelDataWithCommitments].commitments.isIdle(pendingHtlcsOk = true)))
    bob2alice.expectMsgType[Stfu]
    bob2alice.forward(alice)
  ...
  test("recv non-initiator receives second stfu waiting for local commitment to be signed") { f =>
    import f._
    initiateQuiescence(f, sendInitialStfu = false)
