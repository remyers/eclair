...
class PaymentRequestSpec extends AnyFunSuite {
  ...
  test("correctly serialize/deserialize variable-length tagged fields") {
    ...
    val pr = PaymentRequest(chainHash = Block.LivenetGenesisBlock.hash, amount = Some(123 msat), paymentHash = ByteVector32.fromValidHex("01" * 32), privateKey = priv, description = Left("Some invoice"), minFinalCltvExpiryDelta = CltvExpiryDelta(18), expirySeconds = Some(123456), timestamp = 12345)
