...
class Bolt11InvoiceSpec extends AnyFunSuite {
  ...
  test("payment secret") {
    ...
    assertThrows[IllegalArgumentException](
      Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(123 msat), ByteVector32.One, priv, Left("MPP without secrets"), CltvExpiryDelta(18), features = Features(BasicMultiPartPayment -> Optional, VariableLengthOnion -> Optional))
    )
  }
