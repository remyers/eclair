...
object ChannelRelayerSpec {
  ...
  def createLocalUpdate(shortChannelId: ShortChannelId, balance: MilliSatoshi = 10000000 msat, capacity: Satoshi = 500000 sat, enabled: Boolean = true, htlcMinimum: MilliSatoshi = 0 msat): LocalChannelUpdate = {
    ...
    val update = ChannelUpdate(new ByteVector64(randomBytes(64).toArray), Block.RegtestGenesisBlock.hash, shortChannelId, 0, ChannelUpdate.ChannelFlags(isNode1 = true, isEnabled = enabled), CltvExpiryDelta(100), htlcMinimum, 1000 msat, 100, Some(capacity.toMilliSatoshi))
