...
import fr.acinq.eclair.{CltvExpiryDelta, Feature, FeatureSupport, Features, MilliSatoshiLong, NodeParams, TestConstants, TestKitBaseClass, UnknownFeature, randomBytes32, randomKey}
...
class PaymentInitiatorSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("reject payment with unknown mandatory feature") { f =>
    ...
    val pr = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, randomKey(), Left("Some invoice"), CltvExpiryDelta(18), features = Features(Map[Feature, FeatureSupport](Features.VariableLengthOnion -> FeatureSupport.Mandatory, Features.PaymentSecret -> FeatureSupport.Mandatory), Set(UnknownFeature(unknownFeature))))
  ...
  test("forward single-part payment when multi-part deactivated", Tag("mpp_disabled")) { f =>
    ...
    val pr = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some MPP invoice"), finalExpiryDelta, features = Features(VariableLengthOnion -> Mandatory, PaymentSecret -> Mandatory, BasicMultiPartPayment -> Optional))
  ...
  test("forward multi-part payment") { f =>
    import f._
    val pr = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some invoice"), CltvExpiryDelta(18), features = Features(VariableLengthOnion -> Mandatory, PaymentSecret -> Mandatory, BasicMultiPartPayment -> Optional))
    val req = SendPaymentToNode(ActorRef.noSender, finalAmount + 100.msat, pr, 1, CltvExpiryDelta(42), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
  ...
  test("forward trampoline payment") { f =>
    import f._
    val features = Features(VariableLengthOnion -> Mandatory, PaymentSecret -> Mandatory, BasicMultiPartPayment -> Optional, TrampolinePayment -> Optional)
    ...
    val pr = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some phoenix invoice"), CltvExpiryDelta(9), features = features, extraHops = ignoredRoutingHints)
  ...
  test("reject trampoline to legacy payment for 0-value invoice") { f =>
    ...
    val pr = Bolt11Invoice(Block.RegtestGenesisBlock.hash, None, paymentHash, priv_a.privateKey, Left("#abittooreckless"), CltvExpiryDelta(18), None, None, routingHints, features = features)
    val trampolineFees = 21000 msat
  ...
  test("retry trampoline payment") { f =>
    import f._
    val features = Features(VariableLengthOnion -> Mandatory, PaymentSecret -> Mandatory, BasicMultiPartPayment -> Optional, TrampolinePayment -> Optional)
    val pr = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some phoenix invoice"), CltvExpiryDelta(18), features = features)
