...
class ParallelReconnectionTaskSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike with LogCapturing with ParallelTestExecution {

  private val fakeIPAddress = NodeAddress.fromParts("1.2.3.4", 42000).get
  private val fakeIPAddress2 = NodeAddress.fromParts("5.6.7.8", 42001).get
  private val channels = Map(Peer.FinalChannelId(randomBytes32()) -> system.deadLetters.toClassic)

  private val PeerNothingData = Peer.Nothing
  private val PeerDisconnectedData = Peer.DisconnectedData(channels)
  private val PeerConnectedData = Peer.ConnectedData(fakeIPAddress.socketAddress, system.deadLetters.toClassic, null, null, channels.map { case (k: ChannelId, v) => (k, v) })

  case class FixtureParam(nodeParams: NodeParams, remoteNodeId: PublicKey, reconnectionTask: ActorRef[ParallelReconnectionTask.Command], connectionRequestListener: TestProbe[ClientSpawner.ConnectionRequest])
  ...
  override protected def withFixture(test: OneArgTest): Outcome = {
    ...
    if (test.tags.contains("with_node_announcements")) {
      val bobAnnouncement = NodeAnnouncement(randomBytes64(), Features.empty, 1 unixsec, remoteNodeId, Color(100.toByte, 200.toByte, 300.toByte), "node-alias", fakeIPAddress :: fakeIPAddress2 :: Nil)
      aliceParams.db.network.addNode(bobAnnouncement)
    }


    if (test.tags.contains("real_client_spawner")) {
      val untypedSystem = ActorSystem()
      val switchboard = untypedSystem.deadLetters
      val router = untypedSystem.deadLetters
      untypedSystem.actorOf(ClientSpawner.props(aliceParams.keyPair, aliceParams.socksProxy_opt, aliceParams.peerConnectionConf, switchboard, router))
    }

    val connectionRequestListener = TestProbe[ClientSpawner.ConnectionRequest]()
    ...
    LoggingTestKit
      ...
      .expect {
        testKit.system.eventStream ! EventStream.Subscribe(connectionRequestListener.ref)
      }(testKit.system)

    val reconnectionTask = testKit.spawn(ParallelReconnectionTask(aliceParams, remoteNodeId))

    withFixture(test.toNoArgTest(FixtureParam(aliceParams, remoteNodeId, reconnectionTask, connectionRequestListener)))
  }

  test("stay idle at startup if auto-reconnect is disabled", Tag("with_node_announcements")) { f =>
    import f._

    reconnectionTask ! ParallelReconnectionTask.WrappedPeerTransition(Peer.Transition(PeerNothingData, PeerDisconnectedData))
    connectionRequestListener.expectNoMessage()
  }

  test("stay idle at startup if there are no channels", Tag("auto_reconnect"), Tag("with_node_announcements")) { f =>
    import f._

    reconnectionTask ! ParallelReconnectionTask.WrappedPeerTransition(Peer.Transition(PeerNothingData, Peer.DisconnectedData(Map.empty)))
    connectionRequestListener.expectNoMessage()
  }
...
//
  test("initiate reconnection at startup if auto-reconnect is enabled", Tag("auto_reconnect"), Tag("with_node_announcements")) { f =>
    ...
    reconnectionTask ! ParallelReconnectionTask.WrappedPeerTransition(Peer.Transition(PeerNothingData, PeerDisconnectedData))
    connectionRequestListener.expectMessageType[ClientSpawner.ConnectionRequest]
    connectionRequestListener.expectMessageType[ClientSpawner.ConnectionRequest]
  }
