...
class OffersIntegrationSpec extends IntegrationSpec {
  ...
  test("simple offer") {
    ...
    val offer = Offer(Some(100000 msat), "test offer", nodes("A").nodeParams.nodeId, nodes("A").nodeParams.chainHash)
    ...
    val invoice = Bolt12Invoice(offer, invoiceRequest, preimage, dummyInvoice.paymentSecret.get, nodes("A").nodeParams.privateKey, dummyInvoice.minFinalCltvExpiryDelta)
    ...
    val response = probe.expectMsgType[PayOfferResponse]
