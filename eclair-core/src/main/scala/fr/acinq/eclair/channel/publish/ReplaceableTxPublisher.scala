      ...
      Behaviors.withTimers { timers =>
        Behaviors.withMdc(loggingInfo.mdc()) {
          Behaviors.receiveMessagePartial {
            case Publish(replyTo, cmd) => new ReplaceableTxPublisher(nodeParams, bitcoinClient, watcher, replyTo, cmd, context, timers, loggingInfo).checkPreconditions()
