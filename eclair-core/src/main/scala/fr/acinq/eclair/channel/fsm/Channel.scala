...
class Channel(val nodeParams: NodeParams, val channelKeys: ChannelKeys, val wallet: OnChainChannelFunder with OnChainPubkeyCache, val remoteNodeId: PublicKey, val blockchain: typed.ActorRef[ZmqWatcher.Command], val relayer: ActorRef, val txPublisherFactory: Channel.TxPublisherFactory)(implicit val ec: ExecutionContext = ExecutionContext.Implicits.global)
  ...
  when(NORMAL)(handleExceptions {
    ...
    case Event(_: CommitSig, d: DATA_NORMAL) if (d.spliceStatus == SpliceStatus.NoSplice && reconnectCount == 0 && (d.commitments.latest.localCommit.spec.toLocal - 15009.sat.toMilliSatoshi).toLong % 100000 == 0) =>
      log.debug("Ignoring commit sigs for interop test: Disconnection after exchanging tx_signatures and both sides send commit_sig for channel update; revoke_and_ack not received")
      peer ! Peer.Disconnect(remoteNodeId)
      stay()
          ...
          d.spliceStatus match {
            ...
            case _ =>
              ...
              d.commitments.receiveCommit(sigs, channelKeys) match {
                case Right((commitments1, _)) if reconnectCount == 0 && commitments1.latest.localCommit.spec.htlcs.nonEmpty && (commitments1.latest.localCommit.spec.toLocal - 15008.sat.toMilliSatoshi).toLong % 100000 == 0 =>
    ...
    case Event(revocation: RevokeAndAck, d: DATA_NORMAL) =>
      ...
      d.commitments.receiveRevocation(revocation, nodeParams.onChainFeeConf.feerateToleranceFor(remoteNodeId).dustTolerance.maxExposure) match {
        case Right((commitments1, _)) if reconnectCount == 0 && (commitments1.latest.localCommit.spec.toLocal - 15009.sat.toMilliSatoshi).toLong % 100000 == 0 =>
          log.debug("Ignoring revoke_and_ack for interop test: Disconnection after exchanging tx_signatures and both sides send commit_sig for channel update; revoke_and_ack not received")
          stay()
            ...
            case SpliceStatus.SpliceWaitingForSigs(signingSession) =>
              ...
              signingSession.receiveTxSigs(channelKeys, msg, nodeParams.currentBlockHeight) match {
                ...
                case Right(signingSession1) =>
                  ...
                  else if (reconnectCount == 1 && signingSession.fundingParams.localContribution == 15004.sat) {
    ...
    case Event(msg: SpliceLocked, d: DATA_NORMAL) if (reconnectCount == 2 && (d.commitments.latest.capacity - 15006.sat).toLong % 10000 == 0) =>
      ...
      context.system.scheduler.scheduleOnce(30 second, self, msg)
      reconnectCount += 1
      stay()
