  ...
  def start(): Behavior[Command] = {
    ...
    if (hasEnoughFunding) {
      ...
      if (fundingParams.isInitiator) {
        val sharedInput = fundingParams.sharedInput_opt.toSeq.map(sharedInput => Input.Shared(UInt64(0), sharedInput.info.outPoint, 0xfffffffdL, purpose.previousFundingAmount))
        val sharedOutput = Output.Shared(UInt64(0), fundingPubkeyScript, fundingParams.fundingAmount)
      ...
      case inputDetails: InputDetails if inputDetails.unusableInputs.isEmpty =>
        ...
        if (fundedTx.txOut.count(_.publicKeyScript == fundingPubkeyScript) != 1) {
          ...
          val fundingContributions = if (fundingParams.isInitiator) {
            ...
            val fundingOutput = Output.Shared(UInt64(0), fundingPubkeyScript, fundingParams.fundingAmount)
    ...
    currentInputs.find(i => txIn.outPoint == i.outPoint) match {
      ...
      case None => fundingParams.sharedInput_opt match {
        case Some(sharedInput) if sharedInput.info.outPoint == txIn.outPoint =>
          ...
          Future.successful(Right(Input.Shared(UInt64(0), sharedInput.info.outPoint, txIn.sequence, purpose.previousFundingAmount)))
        case _ =>
          for {
