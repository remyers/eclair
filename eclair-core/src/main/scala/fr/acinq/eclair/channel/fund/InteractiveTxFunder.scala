  ...
  def start(): Behavior[Command] = {
    ...
    if (hasEnoughFunding) {
      ...
      if (fundingParams.isInitiator) {
        ...
        val sharedOutput = Output.Shared(UInt64(0), fundingParams.fundingPubkeyScript, purpose.previousLocalBalance + fundingParams.localContribution, purpose.previousRemoteBalance + fundingParams.remoteContribution, htlcsAmount)
      ...
      case inputDetails: InputDetails if inputDetails.unusableInputs.isEmpty =>
        ...
        if (fundedTx.txOut.count(_.publicKeyScript == fundingParams.fundingPubkeyScript) != 1) {
          ...
          val fundingContributions = if (fundingParams.isInitiator) {
            ...
            val htlcsAmount = fundingParams.sharedInput_opt.map(_.info.txOut.amount).getOrElse(0 sat) - purpose.previousLocalBalance - purpose.previousRemoteBalance
            val fundingOutput = Output.Shared(UInt64(0), fundingParams.fundingPubkeyScript, purpose.previousLocalBalance + fundingParams.localContribution, purpose.previousRemoteBalance + fundingParams.remoteContribution, htlcsAmount)
