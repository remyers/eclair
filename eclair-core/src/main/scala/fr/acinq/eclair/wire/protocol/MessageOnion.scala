...
object OnionMessagePayloadTlv {
  ...
  case class InvoiceRequest(request: TlvStream[InvoiceRequestTlv]) extends OnionMessagePayloadTlv

  case class Invoice(invoice: Bolt12Invoice) extends OnionMessagePayloadTlv

  case class InvoiceError(error: TlvStream[InvoiceErrorTlv]) extends OnionMessagePayloadTlv
...
object MessageOnionCodecs {
  ...
  private val onionTlvCodec = discriminated[OnionMessagePayloadTlv].by(varint)
    ...
    .typecase(UInt64(64), variableSizeBytesLong(varintoverflow, invoiceRequestCodec))
    .typecase(UInt64(66), variableSizeBytesLong(varintoverflow, invoiceCodec))
    .typecase(UInt64(68), variableSizeBytesLong(varintoverflow, invoiceErrorCodec))
