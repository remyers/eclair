...
object PaymentOnion {
  ...
  sealed trait RelayPayload extends PerHopPayload with PerHopPayloadFormat {
    ...
    /** public key for the next node to decrypt its payload */
    val nextBlindingKey: Option[PublicKey]
  }
  ...
  case class RelayLegacyPayload(outgoingChannelId: ShortChannelId, amountToForward: MilliSatoshi, outgoingCltv: CltvExpiry) extends ChannelRelayPayload with LegacyFormat {
    override val nextBlindingKey: Option[PublicKey] = None
  }

  case class ChannelRelayTlvPayload(records: TlvStream[OnionPaymentPayloadTlv], blinded: Option[(TlvStream[RouteBlindingEncryptedDataTlv], PublicKey)]) extends ChannelRelayPayload with TlvFormat {
    ...
    override val outgoingChannelId = blinded.map(_._1.get[RouteBlindingEncryptedDataTlv.OutgoingChannelId].get.shortChannelId).getOrElse(records.get[OutgoingChannelId].get.shortChannelId)
    override val nextBlindingKey: Option[PublicKey] = blinded.map(_._2)
  }
  ...
  case class NodeRelayPayload(records: TlvStream[OnionPaymentPayloadTlv], blinded: Option[(TlvStream[RouteBlindingEncryptedDataTlv], PublicKey)]) extends RelayPayload with TlvFormat with TrampolinePacket {
    val amountToForward = records.get[AmountToForward].get.amount
    val outgoingCltv = records.get[OutgoingCltv].get.cltv
    val outgoingNodeId = blinded.map(_._1.get[RouteBlindingEncryptedDataTlv.OutgoingNodeId].get.nodeId).getOrElse(records.get[OutgoingNodeId].get.nodeId)
