...
import fr.acinq.eclair.wire.protocol.OnionMessagePayloadTlv.{Invoice, InvoiceError, InvoiceRequest}
...
import fr.acinq.eclair.{CltvExpiryDelta, Features, MilliSatoshi, TimestampSecond, UInt64}
...
object OfferCodecs {
  ...
  private val features: Codec[FeaturesTlv] = variableSizeBytesLong(varintoverflow, bytes).xmap[Features](Features(_), _.toByteVector).as[FeaturesTlv]

  private val absoluteExpiry: Codec[AbsoluteExpiry] = variableSizeBytesLong(varintoverflow, tu64overflow).as[AbsoluteExpiry]
  ...
  private val payInfo: Codec[PayInfo] = variableSizeBytesLong(varintoverflow,
    ("fee_base_msat" | millisatoshi32) ::
      ...
      ("features" | variableSizeBytes(uint16, bytes).xmap[Features](Features(_), _.toByteVector))).as[PayInfo]
  ...
  val invoiceCodec: Codec[Invoice] = invoiceTlvCodec.narrow({ tlvs =>
    if (tlvs.get[OfferId].isEmpty) {
      Attempt.failure(MissingRequiredTlv(UInt64(4)))
    } else if (tlvs.get[PayerKey].isEmpty) {
      Attempt.failure(MissingRequiredTlv(UInt64(38)))
      ...
      Attempt.successful(Invoice(Bolt12Invoice(tlvs)))
    }
  }, {
    case Invoice(Bolt12Invoice(tlvs)) => tlvs
  })
  ...
  def decodeBech32[A](codec: Codec[A], s: String): Attempt[A] = {
    ...
    if (hrp != "lno") {
      ...
      codec.decode(ByteVector(Bech32.five2eight(data)).bits).map(_.value)
    }
  }
}
