...
import fr.acinq.eclair.{CltvExpiryDelta, Features, MilliSatoshi, TimestampSecond, randomBytes32}
...
object Offers {
  ...
  case class Offer(records: TlvStream[OfferTlv]) {
    ...
    val features: Features = records.get[FeaturesTlv].map(_.features).getOrElse(Features.empty)
  ...
  object InvoiceRequest {
    def apply(offer: Offer, amount: MilliSatoshi, quantity: Long, features: Features, payerKey: PrivateKey, chain: ByteVector32 = Block.LivenetGenesisBlock.hash): InvoiceRequest = {
